(() => {
  const func = (root, initTheme, changeTheme) => {
    ...
    components.onclick = () => {
      ...
      const isDark = htmlElement.classList.contains('dark') ? 'dark' : 'light';
  ...
  class ThemeButton extends HTMLElement {
    ...
    connectedCallback() {
      const nowTheme = this.checkForDarkClass();
      const initTheme = nowTheme;
      const size = +this.getAttribute("size") || 3;
      const shadow = this.attachShadow({ mode: "closed" });
      const container = document.createElement("div");
      container.setAttribute("class", "container");
      container.setAttribute("style", `font-size: ${(size / 3).toFixed(2)}rem`);
      container.innerHTML =
